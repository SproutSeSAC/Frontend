...
export default function ControllerMeetingType() {
  ...
  return (
    <div className="flex flex-col">
      <div
        className={`relative flex items-center rounded-2xl border ${errors?.meetingPlace?.message ? 'border-red-600' : 'border-mainGray'} bg-white [&>div:first-child]:w-44`}
      >
        <Controller
          control={control}
          name="meetingType"
      ...
      <ErrorMsg
        msg={errors?.meetingPlace?.message as string}
        className="pl-2 pt-0"
      />
    </div>
  );
